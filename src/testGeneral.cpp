...
int main ( int argc, char *argv[] ) {
	...
	///*Rotate*/
	Maillage<double, list> M5(2,2, Point<double>(0,0));
	M5.tourner(M_PI/8, Point<double>(1,1));
	...
	/*Transformer*/
	Maillage<double, list> M7(10,10, Point<double>(0,0));

	M7.transformer(0.707, -0.707, 0.707,0.707);
	ofile.open("./generated/7.dat");
	ofile << M7;
	ofile.close();

	Maillage<double, list> M8(Point<double>(2,3), Point<double>(7,3), Point<double>(7,6), Point<double>(2,6), 2,3);
	ofile.open("./generated/8.dat");
	ofile << M8;
	ofile.close();
